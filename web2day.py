# /etc/zivpn/web2day.py — Free Public Panel (minimal, stable)
...
def zivpn_core_delete_user(username):
    ...
    print(f"[ZIVPN] delete would run for {username}")
...
def sort_users(users):
    ...
    def k(u):
        exp = u.get('expires','2099-12-31')
        return (exp < today_str, u.get('status') != 'Online', u.get('user','').lower())
    return sorted(users, key=k)

def check_and_delete_expired_users(users):
    today = datetime.date.today().strftime('%Y-%m-%d')
    expired = [u for u in users if u.get('expires','2099-12-31') < today]
    if expired:
        print(f"[INFO] Deleting {len(expired)} expired users")
        for u in expired:
            if 'user' in u: zivpn_core_delete_user(u['user'])
...
<style>
 :root{--bg:#f8f9fa;--fg:#212529;--muted:#6c757d;--card:#fff;--bd:#dee2e6;--ok:#198754;--bad:#dc3545;--primary:#0d6efd}
 html,body{background:var(--bg);color:var(--fg)} body{font-family:system-ui,Segoe UI,Roboto,Arial,sans-serif;margin:0;padding:12px 12px 70px;min-height:100vh}
 ...
 h1{margin:0;font-size:18px;font-weight:700}.sub{color:var(--muted);font-size:12px}
 ...
 .btn:hover{background:#e9ecef}.btn-primary{background:var(--primary);border-color:var(--primary);color:#fff}.btn-primary:hover{background:#0b5ed7}
 .btn-success{background:var(--ok);border-color:var(--ok);color:#fff}.btn-success:hover{background:#157347}
 ...
 .form-inline{display:grid;grid-template-columns:1fr 1fr;gap:12px}@media(max-width:480px){.form-inline{grid-template-columns:1fr}}
 ...
 .count-box p{margin:0;font-size:13px;color:var(--ok);font-weight:500}.count-box .number{font-size:20px;font-weight:700;color:var(--fg);margin-top:2px;display:block}
 ...
 .status-dot.online{background:var(--ok)}.status-text{color:var(--muted)}.status-text.online{color:var(--ok)}.status-text.expired{color:var(--bad)}
 ...
 .nav-item{text-align:center}.nav-link{display:flex;flex-direction:column;align-items:center;text-decoration:none;font-size:11px;color:var(--muted);font-weight:600}
 .nav-link.active{color:var(--primary)}
</style>
<script>
  function fallbackCopy(t){var a=document.createElement("textarea");a.value=t;a.style.position="fixed";a.style.opacity="0";document.body.appendChild(a);a.focus();a.select();try{var ok=document.execCommand('copy');alert(ok?'ကူးယူပြီးပါပြီ (Legacy): '+t:'ကူးယူမရပါ (Manual copy): '+t);}catch(e){alert('ကူးယူမရပါ (Error): '+t);}document.body.removeChild(a);}
  function copyToClipboard(t,e){if(navigator.clipboard){navigator.clipboard.writeText(t).then(function(){alert('ကူးယူပြီးပါပြီ: '+t)},function(){fallbackCopy(t);});}else{fallbackCopy(t);} if(e) e.preventDefault();}
</script>
</head><body>

{% if info_page %}
  <div class="wrap" style="max-width:500px"><div class="box info-box">
    ...
    <div style="margin-top:16px">
      <label>👤 User Name</label><div class="copy-row"><input type="text" value="{{ info.user }}" readonly><button class="copy-btn" onclick="copyToClipboard('{{ info.user }}', event)">Copy</button></div>
      <label style="margin-top:12px">🔑 Password</label><div class="copy-row"><input type="text" value="{{ info.password }}" readonly><button class="copy-btn" onclick="copyToClipboard('{{ info.password }}', event)">Copy</button></div>
      <label style="margin-top:12px">🌐 VPS IP (Server Address)</label><div class="copy-row"><input type="text" value="{{ info.vps_ip }}" readonly><button class="copy-btn" onclick="copyToClipboard('{{ info.vps_ip }}', event)">Copy</button></div>
      <label style="margin-top:12px">⏰ သက်တမ်းကုန်ဆုံးရက်</label><div class="copy-row"><input type="text" value="{{ info.expires }} ({{ default_expiry_days }} ရက်)" readonly><button class="copy-btn" onclick="copyToClipboard('{{ info.expires }}', event)">Copy</button></div>
      <label style="margin-top:12px">🔌 Port (Device Lock)</label><div class="copy-row"><input type="text" value="{{ info.port }}" readonly><button class="copy-btn" onclick="copyToClipboard('{{ info.port }}', event)">Copy</button></div>
    </div>
...
{% else %}
<header><div class="wrap header-wrap">
  <div style="flex:1"><h1>DEV-U PHOE KAUNT (Free Panel)</h1><div class="sub">ZIVPN User Panel • Total: <span class="count">{{ total }}</span></div></div>
...
<div class="wrap">
{% if filter_type == 'all' %}
<div class="count-box"><p>ရက် ({{ default_expiry_days }}) အတွင်း စုစုပေါင်း အကောင့်ဖွင့်သူ</p><span class="number">{{ total_30_day_users }}</span></div>
...
<div class="box">
  ...
  <form method="post" action="/add">
    ...
    <div class="form-inline-full-width"><button class="btn btn-success" type="submit" style="margin-top:12px;width:100%">Save & Show Info</button></div>
  </form>
</div>
{% endif %}
  ...
  <div class="user-card {% if u.is_expired %}expired{% endif %}">
    <div class="card-header">
      ...
      <div class="status-block">
        {% if u.is_expired %}<span class="status-dot"></span><span class="status-text expired">Expired</span>
        {% elif u.status == 'Online' %}<span class="status-dot online"></span><span class="status-text online">Active</span>
        {% else %}<span class="status-dot"></span><span class="status-text">Inactive</span>{% endif %}
      </div>
    </div>
...
@app.before_request
def _prerq():
    if request.path.startswith('/static') or request.path.startswith('/favicon'): return
    # fresh captcha on index visit
    if request.endpoint == 'index' and 'captcha_answer' in session:
        session.pop('captcha_answer', None)
    # delete expired silently
    users = load_users()
    check_and_delete_expired_users(users)

@app.route('/', defaults={'filter_type': 'all'})
@app.route('/index', defaults={'filter_type': 'all'})  # keep this to avoid arg errors
@app.route('/index/<filter_type>')
def index(filter_type):
    users = load_users()
    today_str = datetime.date.today().strftime('%Y-%m-%d')
    users = check_and_delete_expired_users(users)

    window_ago = (datetime.date.today() - datetime.timedelta(days=DEFAULT_EXPIRY_DAYS)).strftime('%Y-%m-%d')
    total_30 = sum(1 for u in users if u.get('created_on','1970-01-01') >= window_ago)

    for u in users:
        u['expires'] = u.get('expires','')
        u['is_expired'] = u['expires'] < today_str if u['expires'] else False
        u.update(check_user_status(u.get('user','')))
        u['bind_ip'] = u.get('bind_ip')

    users_sorted = sort_users(users)

    if filter_type == 'expired':
        filtered = [u for u in users_sorted if u['is_expired']]
    elif filter_type == 'online':
        filtered = [u for u in users_sorted if u['status'] == 'Online' and not u['is_expired']]
    else:
        filtered = users_sorted
    ...
    return render_template_string(
        ...
        expired_count=get_expired_count(users_sorted),
        online_count=get_online_count(users_sorted),
        today=today_str,
        filter_type=filter_type,
        default_expiry_days=DEFAULT_EXPIRY_DAYS,
        total_30_day_users=total_30,
...
def add_user():
    ...
    ok = False
    try:
        ok = captcha_input and expected is not None and int(captcha_input) == int(expected)
    except Exception:
        ok = False
    if not ok:
    ...
    if not user or not password:
        return redirect(url_for('index', err="User name နှင့် Password အပြည့်အစုံ ထည့်သွင်းပေးပါ။"))

    users = load_users()
    if any(u.get('user') == user for u in users):
        return redirect(url_for('index', err=f"အသုံးပြုသူ {user} သည် ရှိပြီးသား ဖြစ်ပါသည်။"))

    expires = (datetime.date.today() + datetime.timedelta(days=DEFAULT_EXPIRY_DAYS)).strftime('%Y-%m-%d')
    created = datetime.date.today().strftime('%Y-%m-%d')
    # simple random port
    port = random.randint(10000, 20000)

    users.append({'user': user, 'password': password, 'expires': expires, 'created_on': created, 'port': port})
    save_users(users)

    return render_template_string(
        HTML,
        info_page=True,
        info={'user': user, 'password': password, 'vps_ip': VPS_IP, 'expires': expires, 'port': port},
        default_expiry_days=DEFAULT_EXPIRY_DAYS
    )
...
def refresh_status():
    ...
    return redirect(url_for('index', filter_type=f))
