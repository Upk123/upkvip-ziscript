...
def pick_free_port():
    used = {int(u.get("port") or 0) for u in load_users() if u.get("port")}
...
def build_view(msg="", err="", info_user=None):
    users = ensure_expiry_and_prune(load_users())
...
def add_user():
    ...
    port = pick_free_port()
    if not port: return build_view(err="အသုံးပြုရန် UDP port မထိုက်ပါ")
