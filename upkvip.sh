...
# Original: Zahid Islam | Tweaks/MM UI: U Phote Kaunt | One-shot pack by ChatGPT
# Features:
# - Installs ZIVPN binary + systemd
# - DNAT 6000–19999/udp -> 5667 + UFW open + sysctl tune
# - Web UI (Flask) with users.json <-> config.json(auth.config) auto-sync
# - Live Online/Offline (conntrack + socket listen)
# - Expires accepts "YYYY-MM-DD" OR a number of days (e.g. 30)
...
wait_for_apt(){
  for _ in $(seq 1 60); do
    if pgrep -x apt >/dev/null || pgrep -x apt-get >/dev/null || pgrep -f apt.systemd.daily >/dev/null || pgrep -x unattended-upgrade >/dev/null; then
      sleep 5
    else
      return 0
    fi
...
apt_guard_start(){
  ...
  if [ -f "$CNF" ]; then mv "$CNF" "$CNF.disabled"; CNF_DISABLED=1; else CNF_DISABLED=0; fi
}
apt_guard_end(){
  dpkg --configure -a >/dev/null 2>&1 || true
  apt-get -f install -y >/dev/null 2>&1 || true
  if [ "${CNF_DISABLED:-0}" = "1" ] && [ -f "$CNF.disabled" ]; then mv "$CNF.disabled" "$CNF"; fi
}
...
RECENT_SECONDS = 120   # live check window
...
<main>
  <div class="card">
    <p class="tip">users.json ⇄ config.json(auth.config) ကို auto-sync လုပ်ပြီး Online/Offline ကို UDP activity (conntrack) နဲ့စစ်ထားပါတယ်။ <kbd>Expires</kbd> မှာ <b>YYYY-MM-DD</b> သို့ <b>30</b> (ယနေ့+၂ဝရက်) လို နံပါတ်ထည့်လို့ရပါတယ်။</p>
...
echo -e "$LINE"<meta name="viewport" content="width=device-width,initial-scale=1">
<meta http-equiv="refresh" content="120">
<style>
:root{--bg:#0b1020;--card:#121932;--text:#e8ecff;--muted:#9aa4cc;--ok:#30d158;--bad:#ff453a;--acc:#6b8cff}
*{box-sizing:border-box} body{margin:0;font-family:Inter,system-ui,Segoe UI,Roboto,Arial;background:linear-gradient(135deg,#0b1020,#0e1530);color:var(--text)}
