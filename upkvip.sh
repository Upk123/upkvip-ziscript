...
# Extra patch: Live per-user Online/Offline via conntrack + users.json <-> config.json sync
# Extra fix: Add User POST => no redirect, render immediately (10s auto-refresh မပိတ်မကွက်)
...
wait_for_apt() {
  ...
  for i in $(seq 1 60); do
    if pgrep -x apt-get >/dev/null || pgrep -x apt >/dev_null || pgrep -f 'apt.systemd.daily' >/dev/null || pgrep -x unattended-upgrade >/dev/null; then
      sleep 5
    else
      return 0
    fi
...
RECENT_SECONDS = 120       # UDP activity threshold for "Online"
...
echo -e "$LINE"WorkingDirectory=/etc/zivpn
ExecStart=/usr/local/bin/zivpn server -c /etc/zivpn/config.json
Restart=always
RestartSec=3
Environment=ZIVPN_LOG_LEVEL=info
...
<h2>📒 ZIVPN User Panel</h2>
<p class="tip">💡 users.json တင်သက်ဆိုင်ရာ user/password/expires/port ရေးပြီး၊ config.json ရဲ့ auth.config
(passwords list) နဲ့ auto-sync လုပ်ထားပါတယ်။ Online/Offline ကို UDP activity (conntrack) အရ တိကျစွာစစ်ပြီး
socket LISTEN ကို fallback လုပ်ပါတယ် (refresh auto 10s).</p>
...
def has_recent_udp_activity(port: str) -> bool:
    if not port:
        return False
    try:
        # Prefer extended output to read timeout (remaining seconds)
        ext = subprocess.run(
            f"conntrack -L -p udp -o extended 2>/dev/null | grep 'dport={port}\\b'",
            shell=True, capture_output=True, text=True
        ).stdout
        # If any entry shows timeout reasonably high (seen recently), count as Online
        for line in ext.splitlines():
            m = re.search(r'timeout=(\d+)', line)
            if m and int(m.group(1)) >= RECENT_SECONDS // 2:
                return True
        if ext:
            # If we saw entries but no timeout parsed, still treat as recent
            return True
        # Fallback (non-extended)
        out = subprocess.run(
            f"conntrack -L -p udp 2>/dev/null | grep 'dport={port}\\b'",
            shell=True, capture_output=True, text=True
        ).stdout
...
def status_for_user(u, active_ports, listen_port):
    ...
    if has_recent_udp_activity(check_port):
        return "Online"
    # 2) Fallback to socket presence (service running)
    if check_port in active_ports:
        # If user has its own port but no activity => Offline
        # If relying on main listen => show Online (service level)
        return "Offline" if port else "Online"
    return "Unknown"
...
def index():
    ...
    view.sort(key=lambda x: (x.user or "").lower())
    return render_template_string(HTML, users=view, msg=request.args.get("msg",""), err=request.args.get("err",""))
...
def add_user():
    ...
    if not user or not password:
        return redirect("/?err=User+နှင့်+Password+လိုအပ်သည်")

    if expires:
        try: datetime.strptime(expires, "%Y-%m-%d")
        except ValueError:
            return redirect("/?err=Expires+format+မမှန်ပါ+(YYYY-MM-DD)")

    if port:
        if not re.fullmatch(r"\d{2,5}", port):
            return redirect("/?err=Port+မမှန်ပါ")
        p = int(port)
        if not (6000 <= p <= 19999):
            return redirect("/?err=Port+အကွာအဝေး+6000-19999")
    else:
        port = pick_free_port()
    ...
    save_users(users)
    sync_config_passwords()
    return redirect("/?msg=Saved+&+Synced")
