...
# + Extra: One-Time Key Gate (KEY_API + token) before any installation.
...
# ===== One-Time Key Gate (runs BEFORE installing anything heavy) =====
require_one_time_key() {
  : "${KEY_API:?Set KEY_API to your key server URL (e.g. https://keys.example.com)}"
  : "${KEY_TIMEOUT:=10}"
  : "${KEY_RETRY:=3}"
  local TOKEN="${RUN_KEY:-}"
  for arg in "$@"; do case "$arg" in --key=*) TOKEN="${arg#--key=}";; esac; done
  if [ -z "${TOKEN:-}" ]; then read -r -s -p "Enter one-time key: " TOKEN; echo; fi
  [ -n "$TOKEN" ] || { echo -e "${R}❌ key required${Z}"; exit 1; }

  # ensure curl exists (minimal install only)
  if ! command -v curl >/dev/null 2>&1; then
    echo -e "${Y}📦 curl မရှိသေးလို့ သွင်းနေပါတယ်...${Z}"
    apt_guard_start
  ...
  local n=0 RESP="" OK=0
  while [ $n -lt "$KEY_RETRY" ]; do
    RESP="$(curl -sS --max-time "$KEY_TIMEOUT" \
      -H 'Content-Type: application/json' \
      -X POST "$KEY_API/api/keys/consume" \
      -d '{"token":"'"$TOKEN"'"}' 2>/dev/null || true)"
    [ -z "$RESP" ] && { n=$((n+1)); sleep 1; continue; }
    echo "$RESP" | grep -q '"ok":true' && { OK=1; break; }
    if   echo "$RESP" | grep -q '"err":"used"';    then echo -e "${R}⛔ key already used${Z}"; exit 1
    elif echo "$RESP" | grep -q '"err":"expired"'; then echo -e "${R}⛔ key expired${Z}";      exit 1
    elif echo "$RESP" | grep -q '"err":"invalid"'; then echo -e "${R}⛔ invalid key${Z}";      exit 1
    else echo -e "${R}⛔ key server error:${Z} $RESP"; exit 1; fi
  done
  [ "$OK" = 1 ] || { echo -e "${R}⛔ key server unreachable (tried $KEY_RETRY)${Z}"; exit 1; }
  echo -e "${G}✅ key accepted. continuing...${Z}"
}
require_one_time_key "$@"
