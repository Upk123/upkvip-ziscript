...
# Heuristic: Online if there's a conntrack entry with dport=PORT or sport=PORT
# (pre-DNAT external port will appear as dport=PORT). Otherwise if port is open -> Offline. Else Unknown.
def _conntrack_has(port):
  if not port: return False
  # try /proc/net/nf_conntrack (some systems show timeout=.. fields)
  out = sh(f"grep -E \"\\bdport={port}\\b|\\bsport={port}\\b\" /proc/net/nf_conntrack 2>/dev/null | head -n1 || true").stdout
  if out.strip(): return True
  # fallback to conntrack tool
...
def status_for_user(u, active_ports, listen_port):
  ...
  if _conntrack_has(port): return "Online"
  if port in active_ports: return "Offline"
  return "Unknown"

# --------- one-device limit (iptables) ----------
...
def remove_limit_rules(port):
  ...
  for _ in range(20):
    ...
    if not line: break
    ipt(f\"iptables -D INPUT {line.replace('-A ','')}\")  # delete by spec line
